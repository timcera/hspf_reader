    ...
    with pd.HDFStore(hdffile) as store:
        ...
        for att in search:
            ...
            for index in dsnlist:
                ...
                for i in range(psa + 1, psa + 1 + 2 * sacnt, 2):
                    ...
                    if idi not in attrinfo:
                        continue
                    name, atype, length = attrinfo[idi]
                    if atype == "I":
        ...
        for index in dsnlist:
            ...
            for i in range(psa + 1, psa + 1 + 2 * sacnt, 2):
                ...
                ptr = iarray[index + i + 1] - 1 + index
                if idi not in attrinfo:
                    # print('PROGRAM ERROR: ATTRIBUTE INDEX not found', idi, 'Attribute pointer', iarray[index + i+1])
                    continue
